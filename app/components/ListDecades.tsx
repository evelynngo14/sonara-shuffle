...
interface ListDecadesProps {
  decades: DecadeRange;
  onSelect: (decade: string, range: [number, number]) => void;
}

export const ListDecades: React.FC<ListDecadesProps> = ({ decades, onSelect }: ListDecadesProps) => {
  return (
    <ul className="list">
      {Object.entries(decades).map(([label, range]) => (
        <li className="buttons" key={label} onClick={() => onSelect(label, range)}>
          {label}
        </li>
      ))}
    </ul>
